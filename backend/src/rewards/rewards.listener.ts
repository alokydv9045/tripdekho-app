...
export class RewardsListener {
  ...
  async handleUserRegisteredEvent(event: UserRegisteredEvent) {
    try {
      this.logger.log(
        `Handling user.registered event for user: ${event.userId}`,
      );
      ...
      if (!user) {
        this.logger.warn(
          `User ${event.userId} not found, skipping reward generation`,
        );
        return;
      }
      ...
      if (event.referralCode) {
        this.logger.log(
          `User ${event.userId} signed up with referral code: ${event.referralCode}`,
        );
        ...
        for (const rule of rules) {
          ...
          if (
            rule.recipientType === RecipientType.REFERRED ||
            rule.recipientType === RecipientType.BOTH
          ) {
          ...
          // Credit referrer
          if (
            rule.recipientType === RecipientType.REFERRER ||
            rule.recipientType === RecipientType.BOTH
          ) {
            await this.rewardsService.creditPoints(
      ...
      this.logger.error(
        `Failed to handle user registered event for rewards: ${error.message}`,
      );
    }
  }
}

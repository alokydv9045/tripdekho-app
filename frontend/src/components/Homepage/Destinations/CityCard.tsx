...
import { CityDestination } from "@/types/cityDestination";
...
const CityCard: React.FC<CityCardProps> = ({ destination }) => {
  return (
    <Link href={`/search?q=${destination.city}`}>
      <article
        id={`city-card-${destination.id}`}
      ...
      >
        ...
        >
          <Image
            src={fixImageUrl(destination.image?.trim() ? destination.image : "https://images.unsplash.com/photo-1548013146-72479768bbaa?auto=format&fit=crop&q=80&w=400")}

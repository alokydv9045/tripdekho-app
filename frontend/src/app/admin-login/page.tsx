...
import { Eye, EyeOff, Loader2, ShieldCheck, Users, Settings, Database, Activity, Lock, Globe, Server } from 'lucide-react';
...
export default function AdminLoginPage() {
  ...
  const handleAdminLogin = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const result = await authService.login({ email, password, isAdminLogin: true });
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none z-0">
        <div className="absolute top-[12%] left-[8%] animate-fade-in-up delay-300">
          <ShieldCheck className="w-8 h-8 md:w-14 md:h-14 rotate-[15deg] opacity-50 text-[#d97706] animate-float-fast" />
        </div>
        <div className="absolute top-[55%] left-[4%] animate-fade-in-left delay-500">
          <Server className="w-6 h-6 md:w-10 md:h-10 -rotate-12 opacity-40 text-[#d97706] animate-float-slow" />
        </div>
        <div className="absolute top-[25%] left-[25%] animate-fade-in-up delay-400">
          <Activity className="w-10 h-10 md:w-16 md:h-16 rotate-45 opacity-20 text-[#d97706] animate-float-diagonal" />
        </div>
        <div className="absolute bottom-[20%] right-[8%] animate-fade-in-right delay-500">
          <Users className="w-7 h-7 md:w-12 md:h-12 rotate-[10deg] opacity-40 text-[#d97706] animate-float-slow" />
        </div>
        <div className="absolute top-[15%] right-[15%] animate-fade-in-left delay-200">
          <Settings className="w-6 h-6 md:w-10 md:h-10 -rotate-[15deg] opacity-30 text-[#d97706] animate-float-fast" />
        </div>
        <div className="absolute bottom-[15%] left-[20%] animate-fade-in-up delay-400">
          <Database className="w-8 h-8 md:w-12 md:h-12 rotate-[5deg] opacity-30 text-[#d97706] animate-float" />
        </div>
        <div className="absolute top-[35%] right-[5%] animate-fade-in-up delay-500">
          <Globe className="w-10 h-10 md:w-16 md:h-16 opacity-25 text-[#d97706] animate-float-slow" />
        </div>
        <div className="absolute bottom-[25%] left-[45%] animate-fade-in-left delay-300">
          <Lock className="w-8 h-8 md:w-14 md:h-14 -rotate-[10deg] opacity-25 text-[#d97706] animate-float-diagonal" />
        </div>
      </div>
      ...
      >
        ...
        <div className="space-y-2 text-center mb-8">
          <h1 className="text-2xl font-black font-jakarta text-[#191c1d] tracking-tight">Admin Portal</h1>
          <p className="text-[#5e5e5e] text-sm font-semibold">Authorized Personnel Only</p>
        </div>
